...
The heuristics were implemented in C++ and executed on a computer equipped with an Intel Core i3-2120 processor with a frequency of 3.30GHz. The source code for these procedures used for our computational experiments is available at \cite{hawa2018}. We calculated the average optimality of the heuristics for each value of $\delta$ from 1000 instances, where the optimality of each instance is the ratio of the number of strips in the final solution generated by the heuristic to the theoretical minimum $t$, which was calculated using Equation \ref{eqn:lowerbound}. All individual trials were executed in under 160ms.

Tables \ref{table:n=500} and \ref{table:n=1000} compare the results obtained from the three heuristics using different values of $\delta$ and $W$ for $n = 500$ and 1000 respectively. Figures in bold indicate the best average optimality for the given combination of $\delta$ and $W$. Initially, we can see that the optimality ratios tend toward 1 as $\delta$ increases, as the proportion of pairs of score widths that can be packed. 
...
\begin{itemize}
	...
	\item MFFD$^+$ can also find solutions within 1\% of $t$ for $\delta = 0.8$ onwards for all strip widths when $n = 1000$, and the two larger strip widths when $n = 500$.
	\item Number of items per strip smaller, problem is harder, falkenauer 1996, martello and toth 1990b items per strip roughly 2, triplets hardest problems
	...
	\item $W =5000$ Table 2, $\delta = 0.7$ PS has best optimality, however looking at the number of instance solved to $t$, PS has 0, whereas MFFD$^+$ has 152. Therefore there are at least 152 instances where the optimal number of strips = $t$ that PS is not able to solve to optimality.
	\item We see that PS produces solutions using fewer strips on average of the three algoritihms for a larger range of $\delta$ when the strips are wider, and MFFD$^+$ produces solutions using fewer strips for a wider range of $\delta$ when the strips are shorter.
	\item We can see that the range of values of $\delta$ in which PS produces solutions using the minimum number of strips on average of the three algorithms reduces as the strip width decreases
	\item PS never performs best for 0.8 onwards
	\item significant drop between $\delta = 0.4$ and 0.5 for all heuristics and both set sizes except when $W = 1250$
\end{itemize}
