...
\label{alg:mffd+}
\begin{algorithmic}[1]
	\State Sort items in $\mathcal{I}$ in non-increasing order of widths $w_i$
	\For{$i \gets 1 \To |\mathcal{I}|$}	
		...
		\While{\textbf{not} packed}
			\If{$S_j \in \mathcal{S}$ and $\sum_{i = 1}^{|S_j|}w_i + w_i \leq W$}
				\State Run AHC
				\If{AHC returns solution $\mathcal{F}$}
					\State $S_j \gets \mathcal{F}$
					\State packed $\gets \True$
				\Else \hspace{0.5mm} $j \gets j + 1$	
				\EndIf
			\ElsIf{$S_j \notin \mathcal{S}$}
...
\begin{itemize}
	...
	\item All instances completed in less than 850ms
\end{itemize}
