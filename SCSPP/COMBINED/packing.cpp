...
void BasicFFD(int &opt, int &opt90, int &opt80, int &opt70, int &opt60, int &opt50, int &optLow, int numScores, int numItem, int maxItemWidth,
    ...
    for(j = 1; j < itemDecrease.size(); ++j){
        for(i = 0; i < strip.size(); ++i){
            if(!strip[i].empty()){
                if(stripSum[i] + itemWidths[itemDecrease[j]][partners[itemDecrease[j]]] <= stripLength){
...
void PairSmallest(int &opt, int &opt90, int &opt80, int &opt70, int &opt60, int &opt50, int &optLow, int numScores, int stripLength,
    ...
    while (count < numScores){
        for(i = 0; i < scoreIncrease.size(); ++i){
            ...
            if(strip[j].empty()){
                ...
                if(adjMatrix[strip[j].back()][scoreIncrease[i]] == 1 && stripSum[j] + itemWidths[scoreIncrease[i]][partners[scoreIncrease[i]]] <= stripLength){
                    strip[j].push_back(scoreIncrease[i]);
                    strip[j].push_back(partners[scoreIncrease[i]]);
                    stripSum[j] += itemWidths[scoreIncrease[i]][partners[scoreIncrease[i]]];
                    checked[scoreIncrease[i]] = 1;
...
void GGA(int tau, int numScores, int maxItemWidth, int stripLength, vector<int> &allScores, vector<int> &partners, vector<vector<int> > &adjMatrix,
    ...
    if(absentItems.empty()){
        ...
        Mutation(tau, numScores, maxItemWidth, stripLength, allScores, partners, adjMatrix, itemWidths, offspringSum,
                 offspring);
    }
