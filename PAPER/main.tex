...
An optimal solution $\mathcal{S}$ for the SCSPP is a solution that consists of the fewest number of strips $k$ needed to feasibly contain the $|\mathcal{I}|$ items in the given problem instance. A simple lower bound for $k$ is the theoretical minimum
\begin{equation}
\label{eqn:lowerbound}
t = \ceil*{\frac{\sum_{i=1}^{|\mathcal{I}|}w_i}{W}},
\end{equation}
which can be computed in linear time (\citealp{martello1990b}). Perhaps the simplest and most well-known heuristic for packing problems is \textsc{First-Fit} (FF), a greedy online algorithm that places each item presented in some arbitrary order onto the lowest-indexed strip such that the capacity of the strip is not exceeded. It is known that there always exists at least one ordering of the items such that FF produces an optimal solution (\citealp{lewis2009}). A minor modification to FF yields the \textsc{First-Fit Decreasing} (FFD) heuristic, which sorts the items in non-increasing order of size prior to performing FF. In 1973, \citeauthor{johnson1973} showed that FFD is guaranteed to return a solution that uses no more than $\frac{11}{9}k + 4$ strips. More recently, \cite{dosa2007} has proven that the worst case for FFD is in fact $\frac{11}{9}k + \frac{6}{9}$, and that this bound is tight. Due to the initial sorting of the items in non-increasing order of sizes, the time complexity of FFD is $O(n\log n)$, where $n = |\mathcal{I}|$ is the input size.

As mentioned in the introduction, the SCSPP shares many similarities with the BPP, however the addition of constraint (\ref{eqn:vscstrip}) induces complications that must be taken into consideration. One obvious difference is the order in which the items must appear on the strips. The order of the items in the BPP is unimportant, as there are no conditions regarding which items can be placed together, however in the SCSPP the items must be ordered in a way that meets the vicinal sum constraint. In addition, removing an item in the BPP retains feasibility, whereas in the SCSPP this is not guaranteed, as it may leave a subset of items for which the vicinal sum constraint is not satisfied. Furthermore, the theoretical minimum $t$ (Equation (\ref{eqn:lowerbound})) has the potential to be less accurate for the SCSPP, as the minimum scoring distance $\tau$ is not considered. For example, if the minimum scoring distance is greater than twice the largest score width, then it is clear that $|\mathcal{I}|$ strips will be required, regardless of the items' widths.
...
The heuristics were implemented in C++ and executed on a computer equipped with an Intel Core i3-2120 processor with a frequency of 3.30GHz. The tables below show the average optimality of the heuristics for each value of $\delta$ from 1000 instances, where the optimality of each instance is the ratio of the number of strips in the final solution obtained by the heuristic to the theoretical minimum $t$, which was calculated using Equation \ref{eqn:lowerbound}. 

Firstly, it is apparent that the heuristics perform comparitively in the same manner regardless of the input size. 

\begin{itemize}
	\item \textbf{PairSmallest:}
	\item Best average optimality over all three set sizes for $W = 5000$ and $W = 2500$ for $\delta = 0.1 - 0.6$, in some cases up to $\delta = 0.8$.
	\item However, when $W = 1250$, PS only has best average optimality for $\delta = 0.1$ and $\delta = 0.2$. 
	\item PS is able to obtain solution that are on average within 10\% of the lower bound for the optimal solution for $\delta = 0.6$ onwards when $W=5000$ and $W = 2500$.
	\item When $W = 1250$, PS only obtains solutions within 10\% of the lowerbound for $\delta = 0.8$ onwards with set sizes 500 and 1000. For $|\mathcal{I}| = 100$, PS is not able to (on average) obtain solutions within 10\% of the lower bound for any value of $\delta$.
	\item Clear pattern across all three set sizes, PS performs better when the strip widths are wider, however PS is still the best of the three heuristics for smaller strip widths and harder problems, i.e. when $\delta$ is smaller. 
	\item PS never obtains avg solution within 1\% of lower bound.
\end{itemize}


\begin{itemize}
	\item optimality converge toward 1 as $\delta$ increases, i.e. as the problem instances become less difficult.
	\item When $\delta = 1.0$. MFFD and MFFD$^+$ perform identically, as the problems are equivalent to the original BPP, and so they behave like the original FFD heuristic.
	\item When $\delta = 0.0$, all optimalities are equal, as each item requires its own strip, and so $|\mathcal{I}|$ strips are needed for all 1000 instances.
	\item Values in bold represent the best avg optimality of the three heuristics for the given level of $\delta$. 
	\item Clear pattern across both set sizes, PS performs the best out of the three for $\delta = 0.1$ to 0.7 when $W = 5000$, and for $\delta = 0.1$ to 0.6 when $W = 2500$.
	\item However, when $W = 1250$, PS is only superior when $\delta = 0.1$ and 0.2. 
	\item On further inspection, we can see that the difference between the average optimalities for PS and MFFD$^+$ is negligible.
	\item Using Table 1 as an example, when $\delta = 0.2$, the solutions generated by PS are on average within 31.781\% of $t$, and for MFFD$^+$ within 32.61\%, a difference of 0.829\%. 
	\item In practical terms, PS produces solutions using 303.95 strips on average, where $t = 230.648$. MFFD$^+$ produces solutions using 305.862 strips, a difference of less than two strips. 
	\item Another trend is that PS produces average solutions within 10\% of $t$ for $\delta = 0.5$ onwards when $W = 5000$ and 2500, however when $W = 1250$, PS only produces solutions within 10\% of $t$ for $\delta = 0.8$ onwards.
	\item In contrast, MFFD and MFFD$^+$ can produce solution within 10\% of $t$ when $W = 1250$ for $\delta = 0.5$ onwards for $n = 500$, and $\delta = 0.4$ for $n = 1000$.
	\item MFFD$^+$ can also find solutions within 1\% of $t$ for $\delta = 0.8$ onwards for all strip widths in both set sizes.
	\item Number of items per strip smaller, problem is harder, falkenauer 1996, martello and toth 1990b items per strip roughly 2
	\item all instances run in less than 160ms
	\item For each instance where opt $>1$, two cases: either $t$ is incorrect, and there does not exist a solution with $k = t$ strips, and the solution found is actually an optimal solution, or $t$ is correct, and the heuristic cannot find a solution with $t$ strips, not an optimal solution.
	\item since this is a relatively new problem, no problem instances solved to optimality, and so we have no way of knowing if in fact the heuristics are finding optimal solutions. Only able to compare with the theoretical minimum. 
\end{itemize}
...
\begin{table}[!htb]
	\centering
	\ra{1.3}
	\caption{Average optimality ratios for $|\mathcal{I}| = 500$.}
...
\begin{table}[!htb]
	\centering
	\ra{1.3}
	\caption{Average optimality ratios for $|\mathcal{I}| = 1000$.}
	\begin{tabular}{@{}c@{\hspace{25pt}}c@{\hspace{8pt}}c@{\hspace{8pt}}ccc@{\hspace{8pt}}c@{\hspace{8pt}}ccc@{\hspace{8pt}}c@{\hspace{8pt}}c@{}}\toprule
