...
void checkSwap(int &totalCost, int maxStripWidth, vector<vector<int> > &adjMatrix, vector<vector<int> > &boxWidths, vector<vector<int> > &strip, vector<int> &stripSum){
    ...
    cout << "Cost after swapping boxes: " << totalCost << "\n-------------------------\n\n";
...
int costEvalSwap(int i, int j, int k, int l, int totalCost, int maxStripWidth, vector<vector<int> > &boxWidths, vector<vector<int> > &strip, vector<int> &stripSum){
    ...
    newCost = totalCost - pow((maxStripWidth - stripSum[i]), 2) - pow((maxStripWidth - stripSum[k]), 2);
    newSSi = stripSum[i] - boxWidths[strip[i][j]][strip[i][j+1]] + boxWidths[strip[k][l]][strip[k][l+1]];
    newSSk = stripSum[k] - boxWidths[strip[k][l]][strip[k][l+1]] + boxWidths[strip[i][j]][strip[i][j+1]];
    newCost = newCost + pow((maxStripWidth - newSSi), 2) + pow((maxStripWidth - newSSk), 2);
...
void checkSwapTabu(int &c1, int i, int j, int k, int l, int &totalCost, int maxStripWidth, vector<vector<int> > &adjMatrix, vector<vector<int> > &boxWidths, vector<vector<int> > &strip, vector<int> &stripSum){
    ...
    if(strip[i].size() == 2){
        if(l == 0){ //CASE 9
            if(adjMatrix[strip[i][1]][strip[k][2]] == 1){
                c1 = costEvalSwap(i, j, k, l, totalCost, maxStripWidth, boxWidths, strip, stripSum);
            }
            else if(adjMatrix[strip[i][0]][strip[k][2]] == 1){
                c1 = costEvalSwap(i, j, k, l, totalCost, maxStripWidth, boxWidths, strip, stripSum);
            }
...
void packTabu(int c1, int &totalCost, int maxStripWidth, vector<vector<int> > &adjMatrix, vector<vector<int> > &boxWidths, vector<vector<int> > &strip, vector<int> &stripSum){
    ...
    int bestMoveType = 0; //This needs to be inside for loop of iterations for tabu search

    int bestCost = 0; //This needs to be inside for loop of iterations for tabu search
                ...
                for(l = 0; l < strip[k].size()-1; l += 2){
                    if(strip[i].size() == 2 && strip[k].size() == 2){
                        continue;
                            ...
                            && stripSum[k] - boxWidths[strip[k][l]][strip[k][l+1]] + boxWidths[strip[i][j]][strip[i][j+1]] <= maxStripWidth){
                        checkSwapTabu(c1, i, j, k, l, totalCost, maxStripWidth, adjMatrix, boxWidths, strip, stripSum); //this will return a value c1
                        //need to put a check here to see if solution is tabu or not
                        if(c1 > bestCost){
                            bestCost = c1;
                            besti = i;
                            bestj = j;
    ...
    else if(bestMoveType == 1){
        //add previous(current) solution to tabu list
        //perform the swap
        //if bestCost > best cost overall (so far): save bestCost as new best cost overall
        //how to store solution/strips?
        //totalCost = bestCost;
    }
