...
\begin{comment}
\begin{itemize}
	\item need to name this connecting algorithm/method/procedure
	...
	\item should have $|R_i| + |R_j| = q+1$, two edges from one component, one edge from all other components.
	...
	\item multiple sets, cover all components, i.e. combining all edges from sets should mean that each component has at least one edge in the set
	\item each set overlaps with at least one other set, and overlaps with that set only once (not all sets have to overlap with one another)
	\item if such sets found, perform connecting method from above	
\end{itemize}
\end{comment}