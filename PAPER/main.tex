...
\begin{figure}[h!]	
	...
	\caption{Dimensions of an item $i \in \mathcal{I}$ in a regular orientation, and a feasible alignment of two items that can be scored simultaneously.}	
...
The last heuristic incorporates AHCA as described in Section \ref{sec:ahca}. The \textsc{Modified First-Fit Decreasing with AHCA} (MFFD$^+$) heuristic operates in a similar fashion to the MFFD, placing items sorted in decreasing order onto the lowest-indexed strip. However, when the lowest-indexed strip is found that can accommodate the current item, MFFD$^+$ then executes AHCA on the sub-problem, where the items on the strip and the current item form the subset $\mathcal{I}' \subseteq \mathcal{I}$. If AHCA finds a feasible solution, the items are placed on the strip in the order of the solution, which includes the current item, else MFFD$^+$ attempts to pack the current item on the next lowest-indexed strip. 
...
\begin{itemize}
	...
	\item Using AHCA means if a feasible alignment of items inc current items exists (i.e. VSC is met between all items on strip), it will be found (guaranteed)
	\item Unlike in MFFD where an item cannot be placed on the end because VSC is not met, here we can find a solution
	\item items may potentially be entirely rearranged, not in non-decreasing order on each strip as in MFFD (refer to Figure \ref{fig:comparestrips})
	\item Slower as AHCA must be run for each item that can fit on a strip
	\item see Figure \ref{fig:comparestrips}, item cannot be placed onto strip in MFFD heuristic as VSC is not met in either orientation of the item, whereas by using MFFD$^+$ a feasible solution can be found, item can be placed, no need to try another strip or start a new strip
	\item each iteration when an item can fit onto the strip, we have an instance of the sub problem, where the items on the strip and the current items are in the subset $\mathcal{I}' \subseteq \mathcal{I}$ Refer to definition \ref{defn:subprob}.
	\item Then run AHCA on the subproblem, ignore all other strips.
	\item worst case is AHCA is performed for each item on every strip in $\mathcal{S}$ so far (except when new strip needs to be started, AHCA isn't performed, item is simply placed in regular orientation at beginning of the strip).
	\item reduces chances of needing to start a new strip, therefore reducing possibility of increasing number of strips in final solution. 
...
\begin{itemize}
	...
	\item $W = 5000$, Average LB = 11.993, $\sigma$ = 0.56829, Average speed = 38.88504ms/instance
	\item $W = 2500$, Average LB = 23.478, $\sigma$ = 1.05618, Average speed = 38.55795ms/instance
	\item $W = 1250$, Average LB = 46.466, $\sigma$ = 2.02703, Average speed = 37.98091ms/instance
\end{itemize}

\textbf{$n$ = 500}
\begin{itemize}
	\item Average total item width = 287683.855, $\sigma$ = 5434.63046
	\item $W = 5000$, Average LB = 58.039, $\sigma$ = 1.12227, Average speed = 42.11938ms/instance
	\item $W = 2500$, Average LB = 115.571, $\sigma$ = 2.18150, Average speed = 44.24521ms/instance
	\item $W = 1250$, Average LB = 230.648, $\sigma$ = 4.35041, Average speed = 49.66614ms/instance
\end{itemize}

\textbf{$n$ = 1000}
\begin{itemize}
	\item Average total item width = 575163.174, $\sigma$ = 7880.93401
	\item $W = 5000$, Average LB = 115.534, $\sigma$ = 1.59463, Average speed = 214.04479ms/instance
	\item $W = 2500$, Average LB = 230.563, $\sigma$ = 3.16734, Average speed = 190.36579ms/instance
	\item $W = 1250$, Average LB = 460.623, $\sigma$ = 6.30895, Average speed = 214.78729ms/instance
\end{itemize}
